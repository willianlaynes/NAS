...
$sql = 'select * from nurse_box';
$rlt = db_query($sql);
while($line = db_read($rlt)){
  $op1 .= $line['id_bx'].':'.$line['bx_localizacao'].'&';
}



array_push($cp,array('$H8','id_ngb','',False,False));
array_push($cp,array('$O '.$op,'id_ng','Grupo:',True,False));
array_push($cp,array('$O '.$op1,'id_bx','Leito:',True,False));
array_push($cp,array('$O1 1:Ativo&0:Inativo','ngb_status','Status',True,False));
array_push($cp,array('$B8','','Salvar',False,False));

$titulo = 'Cadastro de vinculo GRUPO x LEITO';
$tela = $form->editar($cp,'nurse_grupo_box');
